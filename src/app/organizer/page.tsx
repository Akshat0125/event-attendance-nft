...
import { Calendar, Upload, QrCode, CheckCircle2, Copy, Sparkles, Loader2, X } from 'lucide-react'
...
export default function OrganizerPage() {
  ...
  const fetchEvents = async () => {
    ...
    try {
      // Fetch all Event accounts matching organizer filter
  ...
  return (
    <div className="py-8 max-w-5xl mx-auto px-4 space-y-12">
      {/* Top Banner */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-[#1F2D44] pb-8">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#9945FF]/10 text-[#9945FF] text-xs font-semibold uppercase mb-2 border border-[#9945FF]/20">
            <Sparkles className="w-3.5 h-3.5" /> Organizer Portal
          </div>
          <h1 className="text-3xl sm:text-4xl font-extrabold text-[#E8EDF6]">Manage Events & Badges</h1>
          <p className="text-sm text-[#8FA3C8] mt-1">Create events, choose dynamic badge artwork, and share instant check-in QR codes.</p>
        </div>
        {!wallet.publicKey && <WalletButton />}
      </div>

      {!wallet.publicKey ? (
        <div className="bg-[#161F30] border border-[#1F2D44] rounded-2xl p-10 text-center space-y-4">
          <div className="w-16 h-16 rounded-full bg-[#9945FF]/10 text-[#9945FF] flex items-center justify-center mx-auto">
            <Calendar className="w-8 h-8" />
          </div>
          <h2 className="text-2xl font-bold text-[#E8EDF6]">Connect Organizer Wallet</h2>
          <p className="text-[#8FA3C8] max-w-md mx-auto text-sm">
            Please connect your Phantom wallet to create and manage on-chain events.
          </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-6 bg-[#161F30] border border-[#1F2D44] rounded-2xl p-6 sm:p-8 space-y-6 shadow-xl">
            <div className="flex items-center gap-3 border-b border-[#1F2D44] pb-4">
              <div className="w-10 h-10 rounded-lg bg-[#9945FF]/20 text-[#9945FF] flex items-center justify-center font-bold">
                1
              </div>
              <div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                <label className="text-sm font-semibold text-[#E8EDF6] flex justify-between">
              ...
              <div className="space-y-3">
                <label className="text-sm font-semibold text-[#E8EDF6]">Select Badge Design</label>
              ...
              {/* Custom File Picker UI if custom selected */}
              {selectedBadgeOption === 'custom' && (
                <div className="p-4 bg-[#0B1220] border border-[#1F2D44] rounded-xl space-y-3">
                  ...
                  {isUploading && (
                    <div className="flex items-center gap-2 text-xs text-[#9945FF]">
                      <Loader2 className="w-4 h-4 animate-spin" /> Uploading image to Pinata IPFS...
                    </div>
                  )}
                  {customPreviewUrl && !isUploading && (
                    <div className="flex items-center gap-3 pt-2">
                      <div className="w-12 h-12 rounded-lg overflow-hidden border border-[#9945FF]/40 relative bg-black/40">
                      ...
                      <div className="text-xs">
                        ...
                        <span className="text-[#8FA3C8] text-[10px]">CID: {customUploadedUri?.slice(0, 18)}...</span>
                      </div>
                    </div>
                  )}
                </div>
              ...
              <button
                ...
                className="w-full btn-solana-gradient py-3.5 px-6 rounded-xl font-bold text-base flex items-center justify-center gap-2 disabled:opacity-50"
              >
          ...
          <div className="lg:col-span-6 space-y-6">
            <div className="bg-[#161F30] border border-[#1F2D44] rounded-2xl p-6 sm:p-8 space-y-6 shadow-xl">
              <div className="flex items-center justify-between border-b border-[#1F2D44] pb-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg bg-[#14F195]/20 text-[#14F195] flex items-center justify-center font-bold">
                    2
                  </div>
                  <div>
                    <h2 className="text-xl font-bold text-[#E8EDF6]">Your Events</h2>
                    <p className="text-xs text-[#8FA3C8]">Organized by {wallet.publicKey.toBase58().slice(0, 6)}...</p>
                  </div>
                </div>
                <button
                  onClick={fetchEvents}
              ...
              {isLoadingEvents ? (
                <div className="py-10 text-center text-[#8FA3C8] flex flex-col items-center gap-2">
                  ...
                  {organizerEvents.map((evt) => (
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <h3 className="font-bold text-[#E8EDF6] text-base">{evt.name}</h3>
                          <span className="text-[11px] text-[#8FA3C8] font-mono block">PDA: {evt.publicKey}</span>
                        </div>
                        <span className="px-2.5 py-1 rounded-full bg-[#14F195]/15 text-[#14F195] text-xs font-bold whitespace-nowrap">
                          {evt.attendeeCount} Attendees
                        </span>
                      </div>

                      <div className="flex items-center justify-between pt-2 border-t border-[#1F2D44]/60">
                        <button
                          onClick={() => {
                          ...
                          className="text-xs text-[#8FA3C8] hover:text-[#E8EDF6] flex items-center gap-1.5"
                        >
                        ...
                        <button
                          ...
                          className="btn-solana-gradient px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5"
                        >
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#161F30] border border-[#1F2D44] rounded-2xl p-6 sm:p-8 max-w-md w-full text-center space-y-6 relative shadow-2xl animate-in fade-in zoom-in duration-200">
            ...
            <div className="space-y-1">
              <span className="text-xs uppercase text-[#14F195] font-semibold tracking-wider">Attendee Check-In QR</span>
              <h3 className="text-2xl font-bold text-[#E8EDF6]">{activeQrEvent.name}</h3>
              <p className="text-xs text-[#8FA3C8] font-mono">PDA: {activeQrEvent.pda.slice(0, 10)}...{activeQrEvent.pda.slice(-8)}</p>
            </div>
            ...
            <button
              ...
              className="w-full bg-[#1F2D44] hover:bg-[#283A58] text-[#E8EDF6] py-2.5 rounded-xl text-xs font-semibold flex items-center justify-center gap-2 border border-[#8FA3C8]/20"
            >
